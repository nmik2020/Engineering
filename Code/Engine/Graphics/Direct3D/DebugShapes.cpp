...
void eae6320::Graphics::DebugLine::LoadDebugLine()
{
	...
	s_direct3dDevice->CreateVertexBuffer(bufferSize, usage, useSeparateVertexDeclaration, useDefaultPool,
		&m_vBuffer, notUsed);
...
{
	s_direct3dDevice->SetStreamSource(0, m_vBuffer, 0, sizeof(CUSTOMVERTEX));

	const unsigned int primitiveCount = 1;
	s_direct3dDevice->DrawPrimitive(D3DPT_LINELIST, 0, primitiveCount);
}
...
void eae6320::Graphics::DebugBox::LoadDebugBox()
{
	...
	s_direct3dDevice->CreateIndexBuffer(36 * sizeof(short),
		0,
		D3DFMT_INDEX16,
		D3DPOOL_MANAGED,
		&m_iBuffer,
...
void eae6320::Graphics::DebugBox::DrawBox()
{
	...
	s_direct3dDevice->SetStreamSource(0, m_vBuffer, 0, sizeof(CUSTOMVERTEX));
	s_direct3dDevice->SetIndices(m_iBuffer);

	// draw the cube
	s_direct3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 8, 0, 12);
...
void eae6320::Graphics::DebugSphere::LoadDebugSphere()
{
	...
	i_vertexData.push_back(CUSTOMVERTEX(m_center.x, m_radius, m_center.y, (uint8_t)(m_color.z * 255), (uint8_t)(m_color.y * 255), (uint8_t)(m_color.x * 255)));
	...
	// create an index buffer interface called i_buffer
	s_direct3dDevice->CreateIndexBuffer(m_indexCount * sizeof(unsigned int),
		0,
		D3DFMT_INDEX32,
		D3DPOOL_MANAGED,
		&m_iBuffer,
...
void eae6320::Graphics::DebugSphere::DrawSphere()
{
	...
	// draw the cube
	s_direct3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, m_vertexCount, 0, m_indexCount / 3);
