...
bool eae6320::AssetBuilder::BuildAssets()
{
	...
	// Get $(ScriptDir)
	{
		...
		if ( !eae6320::GetLuaEnvironmentVariable( "ScriptDir", scriptDir, &errorMessage ) )
		{
			wereThereErrors = true;
...
namespace
{
	...
	bool Initialize()
	{
		...
		// Register custom functions
		{
			...
			lua_register( s_luaState, "GetLuaEnvironmentVariable", luaGetEnvironmentVariable );
	...
	int luaGetEnvironmentVariable( lua_State* io_luaState )
	{
		...
		std::string value;
		std::string errorMessage;
		if ( eae6320::GetLuaEnvironmentVariable( i_key, value, &errorMessage ) )
		{
